-- Location Adaptive Modules B3C (populationCulture) -- production read-only pre-flight.
...
-- before 20260906090000_location_population_culture_module.sql is approved for apply: the B3B
-- migration already applied, this migration absent from history, the current allowlist is exactly
-- the pre-B3C four-key array, current row counts, and that no existing location already carries a
-- populationCulture base_profile key this migration's allowlist expansion would suddenly start
-- validating writes against (it wouldn't touch existing data either way -- this is a confirmation,
-- not a precondition for safety).
select 'government_economy_modules_migration_in_history' as check, exists(select 1 from supabase_migrations.schema_migrations where version='20260905090000')::text as value
union all
select 'population_culture_module_migration_already_in_history', exists(select 1 from supabase_migrations.schema_migrations where version='20260906090000')::text
union all
...
union all
select 'project_locations_with_populationCulture_in_moduleSelection_already', count(*)::text from public.project_locations where metadata->'locationProfile'->'moduleSelection'->'shown' ? 'populationCulture' or metadata->'locationProfile'->'moduleSelection'->'hidden' ? 'populationCulture'
union all
