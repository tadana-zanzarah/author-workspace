-- Location Adaptive Modules B3B (governmentSociety, economy) -- post-apply verification.
-- Pure SELECT, single statement, run READ ONLY. Confirms 20260905090000 landed with exactly the
...
union all
select 'allowlist_is_exact_expected_array', (select private.location_thematic_module_keys()=array['appearanceAtmosphere','geography','governmentSociety','economy'])::text
union all
...
union all
-- No automatic governmentSociety/economy keys silently added to any existing row.
select 'locations_with_governmentSociety_key_after', count(*)::text from public.locations where base_profile ? 'governmentSociety'
union all
select 'locations_with_economy_key_after', count(*)::text from public.locations where base_profile ? 'economy'
union all
