...
do $$
declare
  project_id uuid; owner_id uuid;
  loc1_id uuid; loc2_id uuid; scene1_id uuid; scene2_id uuid;
  canonical1 uuid; canonical2 uuid;
  n integer; fk_target text;
begin
  select value::uuid into project_id from public._ci_location_phase2_fixture where key='project_id';
  select value::uuid into owner_id from public._ci_location_phase2_fixture where key='owner_id';
  ...
  if project_id is null or loc1_id is null or loc2_id is null or scene1_id is null or scene2_id is null then
  ...
  --    table.
  select count(*) into n from public.location_projects_legacy_v1 where id in (loc1_id,loc2_id) and project_id=project_id and name='Old Harbor';
  ...
  select location_id into canonical1 from public.project_locations where id=loc1_id and project_id=project_id;
  select location_id into canonical2 from public.project_locations where id=loc2_id and project_id=project_id;
  ...
  select count(*) into n from public.locations where id in (canonical1,canonical2) and name='Old Harbor' and owner_id=owner_id;
  ...
  select count(*) into n from public.scenes where id=scene1_id and location_id=loc1_id and project_id=project_id;
  if n<>1 then raise exception 'scene #1 location_id changed after migration'; end if;
  select count(*) into n from public.scenes where id=scene2_id and location_id=loc2_id and project_id=project_id;
...
do $$
declare
  project_id uuid; loc1_id uuid; scene1_id uuid;
...
begin
  ...
  r:=public.update_location(project_id,loc1_id,rev,'Old Harbor','Repaired after the Phase 2 migration.');
  ...
  r:=public.delete_location(project_id,loc1_id,rev);
  ...
  r:=public.update_scene(project_id,scene1_id,rev,null,null,'Scene At Copy 1','',null,null,'placed','draft',true,false);
