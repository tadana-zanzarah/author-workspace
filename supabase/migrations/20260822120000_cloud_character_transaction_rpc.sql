...
begin
  ...
  update public.characters set name=btrim(character_name),surname=coalesce(character_surname,''),base_profile=base_profile,revision=revision+1 where id=target_character_id returning * into item;
...
begin
  ...
  if exists(select 1 from jsonb_to_recordset(relations) x(from_project_character_id uuid,to_project_character_id uuid,value_operation text,value text,visible boolean,metadata jsonb) group by 1,2 having count(*)>1) then return jsonb_build_object('ok',false,'code','DUPLICATE','revision',p.revision,'message','Duplicate directed relation.','changed',false); end if;
