...
try{
  ...
  const scroll=await page.evaluate(()=>{commitDataChange(next=>{for(let i=0;i<45;i++)next.scenes.push({id:`long-${i}`,title:`Long ${i}`,date:"",time:"",dateReview:false,chapterId:"chapter-two",locationId:"",tags:[],writingStatus:"idea",sceneText:"",included:true,status:"floating",people:{}})});currentView="list";render();const viewport=document.querySelector(".workspace-viewport"),handle=document.querySelector(".compact-drag-handle");compactDragStart({currentTarget:handle,preventDefault(){},dataTransfer:{effectAllowed:"",setData(){}}},handle.closest("[data-scene-id]").dataset.sceneId);viewport.scrollTop=0;const rect=viewport.getBoundingClientRect();for(let i=0;i<20;i++)autoscrollSceneViewport(rect.bottom-2);const down=viewport.scrollTop;viewport.scrollTop=viewport.scrollHeight;for(let i=0;i<20;i++)autoscrollSceneViewport(rect.top+2);return {down,up:viewport.scrollTop<viewport.scrollHeight-viewport.clientHeight}});
