...
function createMultiValueCombobox({host,suggestions=[],values=[],placeholder="Добавить…",label="Значения",onChange=()=>{}}){
  ...
  // Location Manual UX Batch B, B8: the INITIAL suggestions array shape decides whether this
  // field gets a dropdown chevron/toggle at all. An empty suggestions array (every Location tag
  // field today) means there is nothing to browse -- rendering a "▾" there misleadingly promises a
  // list that will never have anything in it. A non-empty array (Character favorites/hobbies) is
  // unaffected: same chevron/toggle/list behavior as before, verbatim.
  const hasSuggestions=suggestions.length>0;
  const html=value=>String(value).replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[char]);
  const listId=`${host.id}-listbox`;
  const toggleHtml=hasSuggestions?`<button type="button" class="multi-value-toggle" aria-label="Открыть список: ${label}" tabindex="-1">▾</button>`:"";
  host.innerHTML=`<div class="multi-value-chips"></div><div class="multi-value-control"><input type="text" role="combobox" aria-label="${label}" aria-autocomplete="list" aria-expanded="false" aria-controls="${listId}" autocomplete="off" placeholder="${placeholder}">${toggleHtml}</div><div id="${listId}" class="multi-value-list" role="listbox" hidden></div>`;
  const chips=host.querySelector(".multi-value-chips"),input=host.querySelector("input"),toggle=host.querySelector(".multi-value-toggle"),list=host.querySelector("[role=listbox]");
  ...
  function renderList(){
    ...
    // A field with no initial suggestions never shows an empty listbox -- only once typing
    // produces a real "Добавить «query»" candidate (see `filtered` above). Fields WITH suggestions
    // keep the exact old behavior (visible whenever open, empty or not).
    const visible=open&&(hasSuggestions||filtered.length>0);
    list.hidden=!visible;input.setAttribute("aria-expanded",String(visible));
